(function(window) {
    ...
    function addClassSVG($el, className) {
        ...
        $el.attr('class', classes + className);
    }

    function addCSSTransform($el, transform) {
        var css = '',
            prefixes = ['-moz-', '-webkit-', '-ms-', '-o-', ''];

        for (var i = 0; i < prefixes.length; i++) {
            css += prefixes[i] + "transform" + ": " + transform + '; ';
        }

        $el.attr('style', css);
    }

    function svgTranslateToCss(translate) {
        var values = translate.substring(translate.indexOf('(') + 1, translate.indexOf(')')),
            x = values.substring(0, values.indexOf(',')) || values,
            y = values.substring(values.indexOf(',')+1, values.length);

        return 'translate(' + ((x === y) ? x+'px,'+y+'px' : x+'px') + ')';
    }

    var Widget = {

        baseRotate: ' rotate(0deg) ',
        baseScale: ' scale(1) ',
        centerRotate: ' rotate(30deg) ',
        centerScale: ' scale(0) ',
        centerTranslate: '',

        switchCharts: function() {
            $('g').each(function(i, el) {
                addCSSTransform($(el), this.centerRotate + this.centerScale + this.centerTranslate);
            }.bind(this));
        },
        ...
        init: function() {
            ...
            this.centerTranslate = ' translate(' + chartWidth*2 + 'px, ' + chartHeight/2 + 'px)';
            ...
            for (var q = 0; q < titles.length; q++) {
                ...
                addCSSTransform($g, this.baseRotate + this.baseScale + ' translate(' + legendWidth*(q/titles.length) + 'px,' + legendSectionSize + 'px)');
            ...
            for (var i = 0; i < days.length; i++) {
                ...
                addCSSTransform($g, this.baseRotate + this.baseScale + ' translate(' + xOffset + 'px)');
